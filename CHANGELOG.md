...
- **Onboard recording produced 0-byte `.ts` files** — the recorder's gst-launch-1.0 pipeline included `rtpjitterbuffer`, which is meant to smooth network jitter/reordering but has nothing to do on the recording tap's loopback (127.0.0.1) source; as a standalone element it stalled after a handful of buffers on clock/latency negotiation, starving `mpegtsmux` downstream. Removed it from the pipeline in `VideoRecorder.cpp`.
- **Recording silently wrote to the Pi's root filesystem instead of the USB drive** — `checkMediaWritable()` only checked that `mount_path` existed and was writable, not that it was an actual mount point. If the USB drive was unplugged or not yet mounted, `/mnt/usb` still existed as a plain (writable) directory on the root filesystem, so `rec.start` "succeeded" and recorded there — the drive was empty when later inspected on another machine. `checkMediaWritable()` now also compares `mount_path`'s device id against its parent directory's; a match (nothing mounted on top) now returns `err:no_media` instead of silently recording to the wrong filesystem.
- **Recording could come back at 0 bytes after a hard power loss** — the recording file's writes sat in the page cache for an unbounded time before the kernel flushed them, and on FAT/exFAT (no journal) that includes the directory-entry size update itself: confirmed in the field, pulling power mid-recording left a `.ts` at exactly 0 bytes despite MPEG-TS's truncation tolerance, because the data never reached the drive at all. `VideoRecorder` now runs a periodic global `sync()` while recording is active (`fsync_interval_sec`, default 3s) plus a final `sync()` the moment recording stops, bounding worst-case tail loss to roughly that interval instead of "however long the kernel felt like buffering." Docs now recommend formatting the recording drive ext4 (journaled) instead of FAT32/exFAT, with a udev + systemd `.mount` recipe so the drive self-remounts after a reseat or reboot with no operator action — this stays a single continuous file for the whole recording session, not split into segments.
- **Recording still came back at 0 bytes if media was pulled within the first `fsync_interval_sec`** — the periodic sync thread's first tick waited a *full* interval before its first `sync()`, so on ext4 (delayed allocation: nothing is durably placed on disk until the first flush) a recording interrupted in its opening seconds had nothing committed yet, reproducing the exact 0-byte failure the periodic sync exists to prevent — confirmed in the field by pulling the drive shortly after `rec.start`. The first tick now fires after `min(fsync_interval_sec, 1)` seconds instead of a full interval.
