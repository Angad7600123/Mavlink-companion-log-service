...
- While active, a detached thread calls the global `sync()` every `fsync_interval_sec` seconds (default 3, `0` disables) — except the very first call, which fires after `min(fsync_interval_sec, 1)` seconds rather than a full interval, since ext4's delayed allocation means a freshly-started recording has *nothing* durable yet until the first sync ever runs; without the shortened first tick, media removed in the first couple seconds of a recording reproduced the same 0-byte result this mechanism exists to prevent. The recording file is written by the gst-launch-1.0 subprocess, not mcls itself, but `sync()` flushes all dirty pages system-wide regardless of which process dirtied them — this is what actually bounds power-loss data loss; MPEG-TS's truncation tolerance only helps for bytes that made it to the physical device.
- **A live media removal while the Pi keeps running is a harsher test than a full power loss**, and behaves differently: on a full power cut, the recorder, the sync thread, and the drive all stop together at the same instant, so whatever the last completed sync landed is safely on disk. Yanking just the drive while the OS keeps running risks a mid-write I/O error at an arbitrary moment instead. For a drone, secure the USB connector against vibration-induced disconnection (strain relief / locking cable) — no software mitigation removes the risk of storage vanishing mid-write, only shrinks the window.
